...
import { skillsAnim, leftAnim, rightAnim, skillCatAnim } from "./Animations/skillsAnimations";
        ...
        <motion.section className='skills' variants={skillsAnim} initial="init" animate="anim">
            <motion.div className="leftSide" variants={leftAnim}> 
                {skillCat.map((skill,i) => {
                    return (
                        <div key={i} className="skillsContainer" onMouseEnter={() => skillMouseEnter(i)} onMouseLeave={() => skillMouseLeave()}>
                            {skill.map(el => <SkillItem key={el.skillName} icon={el.icon} title={el.skillName} xp={el.xp}/>)}
                        </div>
                    )
                })}
            </motion.div>
            ...
            <motion.div className="rightSide" variants={rightAnim}>
                <AnimatePresence initial={false}>
                    {hardSkillIndex===0?
                        <motion.h1 key={`s${softSkillIndex}`} className="catName" variants={skillCatAnim} initial="hidden" animate="visible" exit="exit">{skill[0].name[softSkillIndex]}</motion.h1>
                        :
                        <motion.h1 key={`h${hardSkillIndex}`} className="catName" variants={skillCatAnim} initial="hidden" animate="visible" exit="exit">{skill[hardSkillIndex].name}</motion.h1>
                    }
                </AnimatePresence>
