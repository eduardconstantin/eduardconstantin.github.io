...
import { menuContentAnim, menuIconElemAnim, socialAnim, menuIcon, menuAnim } from "./Animations/menuAnimations";
    ...
    return (
        <motion.section className="menuSection" variants={menuAnim} initial="init" animate="anim">
            <div className="locationContainer">
                <p>Located in <span>Romania</span></p>
            </div>

            <motion.div className="menuContainer">
                <motion.div className="menuIcon" onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)} variants={menuIcon} initial="closed" animate={isOpen ? "open" : "closed"}>
                ...
                <motion.div variants={menuContentAnim} initial="closed" animate={isOpen ? "open" : "closed"} onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
                    ...
                    <div className="menu">
                        <ul className="menuList">
                            <li>
                                ...
                                onMouseLeave={activeMenu} onClick={() => selectPage(0)}>ABOUT ME</p>
                            </li>
                            <li>
                                <p ref={skillRef} 
                                onMouseEnter={(event) => menuHover(event.currentTarget)} 
                                onMouseLeave={activeMenu} onClick={() => selectPage(1)}>SKILLS</p>
                            </li>
                            <li>
                                <p ref={projRef} 
                                onMouseEnter={(event) => menuHover(event.currentTarget)} 
                                onMouseLeave={activeMenu} onClick={() => selectPage(2)}>PROJECTS</p>
                            </li>
                        </ul>
                    </div>
