...
export default function App() {
  ...
  const generateBackground = () =>{
      ...
      for (let i = 0; i < icons; i++) {
          iconsArray.push({id: i, src: backgroundIcons[Math.floor(Math.random()*backgroundIcons.length)], opacity: Math.random()})
      }
          ...
          {hasLoaded?
            (<motion.div key="content">
              <motion.div variants={iconContainerAnim} initial="end" className="iconsContainer">
                {bg.map(({id, src, opacity}) => <motion.div variants={iconsAnim} custom={Math.random()} key={id} className='iconWrapper' style={{width:iconSize, height:iconSize}}><img src={src} width={52} style={{opacity: opacity}}/></motion.div>)}
              </motion.div>
              <AnimatePresence exitBeforeEnter>
